...
VotingMatrix::VotingMatrix(vector<vector<vector<MatrixNode*>>> matrix_in){//exclusively for testing with stubs
m_width=matrix_in.size();
m_height=matrix_in[0].size();
m_depth=matrix_in[0][0].size();
matrix=matrix_in;
}
...
MatrixNode *VotingMatrix::createElement(int i_wid, int j_hei, int k_dep) {
    ...
    if (u == 'x') {//true for all but the left and right faces
        ...
        if (v == 'y') {// true for front and back views
            ...
            uint8_t weight=m_image.getWeight(i_wid,j_hei);
            uint8_t *colors = m_image.getValue(i_wid, j_hei);

            m_image.setMaxDepth(full_depth);
            unsigned int depth_of_peak=m_image.getDepthOfPeak(i_wid,j_hei);  //depth will be how far in this goes into the image from 0 to imagelength-2 (need at least one space because opposite sides cant cross over)
            ...
            uint8_t weight=m_image.getWeight(i_wid,k_dep);

            uint8_t *colors = m_image.getValue(i_wid, k_dep);

            m_image.setMaxDepth(full_height);
            unsigned int depth_of_peak=m_image.getDepthOfPeak(i_wid,k_dep);  //depth will be how far in this goes into the image from 0 to imagelength-2 (need at least one space because opposite sides cant cross over)
      ...
      //      std::cerr << "error: image has height of " << image_height << "trying to project on a cube's height of" << m_height << std::endl;
        ...
        uint8_t weight=m_image.getWeight(k_dep,j_hei);

        uint8_t *colors = m_image.getValue(k_dep, j_hei);


        m_image.setMaxDepth(full_width);
        unsigned int depth_of_peak=m_image.getDepthOfPeak(k_dep,j_hei);  //depth will be how far in this goes into the image from 0 to imagelength-2 (need at least one space because opposite sides cant cross over)
